  ...
  const fetchBills = async () => {
    ...
    try {
        ...
        if (isRecall) {
            ...
            // Get completed sales for returns
            res = await posService.getSales(branchId, { status: 'COMPLETED', limit: 50 });
        }
        ...
        const mappedData = (Array.isArray(rawData) ? rawData : []).map(item => ({
            ...
            invoiceNo: item.invoiceNo || item.invoice_no || `INV-${item.saleId || item.id}`,
            saleDate: item.saleDate || item.sale_date || item.createdAt,
            customerName: item.customerName || item.customer?.name || 'Walk-in Customer',
            customerId: item.customerId || item.customer_id,
            itemCount: item.itemCount || item.items?.length || item.item_count || 0,
            ...
            cashierName: item.cashierName || item.cashier?.name || '',
            items: item.items || [],
            ...
            {!loading && !error && filteredData.length > 0 && (
                <div className="divide-y divide-slate-100">
                    {filteredData.map((item) => (
                        <div 
                            key={item.id} 
                            className="flex justify-between items-center p-4 hover:bg-slate-50 cursor-pointer transition-colors group"
                            onClick={() => onSelect(item)}
                        >
                            <div className="flex-1">
                                {/* Invoice Number & Status */}
                                <div className="flex items-center gap-2 mb-1">
                                    <span className="font-bold text-slate-800 font-mono">{item.invoiceNo}</span>
                            ...
                            <div className="text-right ml-4">
                                ...
                                <button className={`mt-1 text-[10px] px-3 py-1.5 rounded font-bold uppercase tracking-wide transition-colors ${
                                    isRecall 
